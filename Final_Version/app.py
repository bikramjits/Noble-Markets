#For Noble Markets New York.
#Sources cited in the end.
...
class LoginHandler(BaseHandler):
    ...
    def post(self):
        ...
        if data_verify(getusername, getpassword):
            input_user = session.query(Balance).filter(Balance.username == getusername).first()
   ...
   def post(self):
    ...
    if to_user is None:
        ...
        if float(curr_user.trading_balance) >= trade_amount:
            fin_amt = (float(curr_user.trading_balance) - trade_amount)
            curr_user.trading_balance = str(fin_amt)
        else:
...
1. Tornado User Authentication Example By mehmetkose
	https://github.com/mehmetkose/tornado-user-authentication-example

2. Structure of Tornado Web Application
	http://www.tornadoweb.org/en/stable/guide/structure.html

3. Introductory Tutorial of Python’s SQLAlchemy By Python Central
